/*
* AidentitDF 0.1.17 - Modern & flexible browser fingerprint library
* 
...
(function (name, context, definition) {
  ...
  DF.prototype = {
    ...
    log: function(msg){
      ...
      // all keys that include versions will be splittet into sub versions                                                    Entropy
      // keys = this.UAK(keys); 1.User Agent Key                                                                              (11.5)
      // keys = this.CSK(keys); 2.Client Storage Key (sessionStorage, localStorage, indexedDb, addBehaviour, open database)
      // keys = this.TLK(keys); 3.Time language Key (language, Timezoneoffset)                                                (10.04, 3.04)
      // keys = this.DPK(keys); 4.Device properties Key (colorDepth, screenResolution, cpuClass, )                            (8.8,
      // keys = this.DHK(keys); 5.Device Hardware Key (canvas, weggl)                                                         (10.8, )
      // keys = this.DPK(keys); 6.Device Protection Key (Content Filter and Proxy - adblocker, VPN )
      // keys = this.DEK(keys); 7.Device environment Key (plugins, fonts)                                                     ( , 13.9)
      // keys = this.STK(keys); 8.Stack trace key (webRTC, ?)
      // keys = this.NWK(keys); 9.Network and Latency 
      
      // keys = this.UBK(keys); 10.User biometric keys (keystroke, scroll accel, touch & mouse path)
      // 1. fixed stuff is hashed, version are hased in a sepcial fuzzy way
      // 2. normal hash
      // 3. language is hashed timezone hashed in a fuzzy way in order to cope with different locations
      // 4. normal hash (problem multi screen)
      // 5. normal hash
      // 6. normal hash
      // 7. fuzzy hash
      // 8. normal hash
    ...
    fontsKey: function(keys, done) {
      if(this.options.excludeFlashFonts) {
        ...
        if(this.options.excludeJsFonts) {
          if(DEBUG) {
            this.log("SJFD");
          } //Skipping js fonts detection per excludeJsFonts configuration option
          return done(keys);
        }
